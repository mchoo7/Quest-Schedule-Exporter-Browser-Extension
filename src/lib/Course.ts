...
export class Course {
	...
	constructor(
		...
		const daysWithClasses = classDays.replace(/Th/g, 'H').toLowerCase();
		...
		const firstDateOfClass: Date = (function () {
			...
			const daysWithClassesNumbers = daysWithClasses
				.split('')
				.map((d) => weekdayIndex[d as Weekday]);

			let dayCounter = 0;
			const MAX_DAY_ADVANCEMENT = 365;

			while (
				!daysWithClassesNumbers.includes(firstDate.getDay()) &&
				dayCounter++ < MAX_DAY_ADVANCEMENT
			) {
	...
	*printer(summary: string, description: string): Generator<string> {
	...
	fillPlaceholders(template: string): string {
		...
		for (const placeholder of this._config.placeholders) {
